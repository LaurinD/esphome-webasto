...
class Webasto : public Component, public uart::UARTDevice {
    ...
    // the base class UARTDevice already keeps a pointer to the parent
    // uart::UARTComponent via its protected member `parent_`
  ...
  public:
    ...
    void SendBreak() {
      ...
      if (now - last_ok_rx >= send_break_periode) {
        ...
        // empty rx buffer
        uint8_t waste;
        while (this->available() > 0) this->read_byte(&waste);

        // send magic byte with slow baudrate for 25ms LOW level
        // change baud rate using generic API and re‑apply settings
        this->set_baud_rate(300);
        this->load_settings(false);
        this->write_byte(0b10000000);
        this->flush();
        ...
        while ((millis() - rxstr) < 50) {
          if (this->available()) {
            this->read_byte(&waste);
            break;
          }
          else delay(1);
        }

        // restore baudrate-to-normal
        this->set_baud_rate(2400);
        this->load_settings(false);
      } else {
    ...
    bool tx_msg2(uint8_t* dat, uint8_t len) {
      ...
      while (this->available() > 0) this->read_byte(&waste);
      // Send message
      this->write_array(&txbuf[0], txcnt);
      this->flush();
      // Receive own message
      uint8_t rxbuf[40];
      uint8_t rxcnt = 0;
      ...
      while (rxcnt < txcnt && (millis() - rxstr) < 100) {
        if (this->available()) this->read_byte(&rxbuf[rxcnt++]);
        else delay(1);
      }
      logbuf[0] = '\0';
